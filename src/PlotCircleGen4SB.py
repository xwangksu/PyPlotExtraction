...
ap.add_argument("-s", "--sourcePoints", required=True, help="source points")
ap.add_argument("-f", "--fieldFile", required=True, help="field layout")
ap.add_argument("-t", "--tFile", required=True, help="target file")
args = vars(ap.parse_args())
src = args["sourcePoints"]
shpf = args["fieldFile"]
tgtf = args["tFile"]

originalPoints = pd.read_csv(src, header = 0, usecols=[0,1,2])
# print(originalPoints)
# Only two points in Long and Lat
...
x4 = originalPoints.iloc[1][0]
y4 = originalPoints.iloc[1][1]
# print(x4)
# print(y4)
...
#
# 4
fieldmap = pd.read_csv(shpf, header = None)
# fieldmap.iloc[0][0]
...
def cal_kb(a1,b1,a2,b2):
    if a1 != a2:
        ...
        print('Pure straight line detected!!')
        k=np.nan
        b=np.nan
    return k,b
...
# cpsy
import csv
finalFile = open(tgtf,'wt')
try:
    writer = csv.writer(finalFile, delimiter=',', lineterminator='\n')
    writer.writerow(('Plot_ID','X1','Y1','X2','Y2','X3','Y3','X4','Y4'))
    for i in range(rowNum):
        for j in range(colNum):
            writer.writerow((fieldmap.iloc[i][j],cpsx[i,j],cpsy[i,j],cpsx[i+1,j],cpsy[i+1,j],cpsx[i+1,j+1],cpsy[i+1,j+1],cpsx[i,j+1],cpsy[i,j+1]))
finally:
    finalFile.close()
